...
function afterMetaReplace(instance) {
    var msg = instance.options.dryRun ? 'Should replace' : 'Replaced';
    console.log(msg, 'metadata in', instance.rawfile);
}

function afterMetaDeleted(instance) {
    var msg = instance.options.dryRun ? 'Should delete' : 'Deleted';
    console.log(msg, 'metadata in', instance.rawfile);
}
...
function testInstance(instance) {
    ...
    if (hasNonPathConfig(loaderConfig)) {
        ...
        if (hasRequiresConfig(loaderConfig) && (
            ...
            return instance.replaceMeta(loaderConfig).tap(afterMetaReplace);
        } else {
        ...
        if (hasConfig(metadataNode, isNonEmptyPropertyValueArray)) {
            ...
            return instance.deleteMeta().tap(afterMetaDeleted);
        }
    }
...
exports.interpret = function interpret() {
    ...
    // options passed to constructors do not need every CLI option
    ...
    var loaderConfigPromise = Promise.map(configs, function (filename) {
        return new YuiGroup(filename, opts).promise;
    })
    ...
    var moduleInstancesPromise = Promise.map(modules, function (filename) {
        return new YuiModule(filename, opts).promise;
    })
