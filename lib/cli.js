...
function testInstance(instance) {
    ...
    header.push(groupName, moduleName, instance.rawfile);
    ...
    if (hasNonPathConfig(loaderConfig)) {
        ...
        if (hasRequiresConfig(loaderConfig) && (
            ...
            if (configBlock !== loaderBlock) {
                throw new ConfigMismatch(header, loaderBlock, configBlock);
            }
        }
        ...
        if (hasConfig(metadataNode, isNonEmptyPropertyValueArray)) {
            ...
            throw new ConfigMismatch(header, null, generateConfigBlock(instance, metadataNode));
        } else {
...
exports.interpret = function interpret() {
    ...
    var loaderConfigPromise = Promise.map(configs, function (filename) {
        ...
        .tap(function () {
            endedConfig = ((Date.now() - beginConfig) / 1000).toFixed(2);
        });


    var beginFiles = Date.now(),
        endedFiles;
    ...
    var moduleInstancesPromise = Promise.map(modules, function (filename) {
        ...
        .tap(function () {
            endedFiles = ((Date.now() - beginFiles) / 1000).toFixed(2);
        });

    // rewrite matches, injecting config blocks
    Promise.join(loaderConfigPromise, moduleInstancesPromise, function (loaderConfigs, instances) {
        ...
        .tap(function () {
            console.log('parsed config files in %ss', endedConfig);
            console.log('parsed module files in %ss', endedFiles);
            console.log(' execution finished in %ss', ((Date.now() - started) / 1000).toFixed(2));
        });
};
